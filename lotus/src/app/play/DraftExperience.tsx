...
const VERSION_BLURBS: Record<string, string> = {
  ...
  v4_card_quality: 'Also learns what a strong card looks like',
};

const versionLabel = (version: string) => VERSION_LABELS[version] ?? version.replace(/_/g, ' ');
...
export interface DraftExperienceProps {
  /** Locks the draft to one set instead of reading the player's chosen set. */
  lockedSetCode?: string;
  /** Which header tab renders as active. */
  activeTab?: string;
  /**
   * Shows a row of model boxes above the pack. Every loaded model ranks the
   * pack, each box reports that model's own top pick, and clicking one puts
   * its numbers on the cards. Off by default, where the settings dropdown
   * picks the single model that answers.
   */
  showModelBoxes?: boolean;
  /** Rendered above the pack, for a page that needs to explain itself. */
  intro?: React.ReactNode;
}

// --- Shared draft experience: /play and /hobbit both render this ---
export function DraftExperience({
  lockedSetCode,
  activeTab = 'sets',
  showModelBoxes = false,
  intro,
}: DraftExperienceProps) {
  ...
  const [currentSet, setCurrentSet] = useState(() => {
    if (lockedSetCode) return lockedSetCode;
  ...
  // When the model boxes are on, every model ranks the pack so each box can
  // report its own top pick. The cards themselves show whichever model is
  // selected, so the grid never has to carry three sets of numbers.
  const [modelPredictions, setModelPredictions] = useState<Record<string, Prediction[]>>({});
  const comparisonAbortController = useRef<AbortController | null>(null);

  useEffect(() => {
    if (!showModelBoxes || boosterCards.length === 0 || availableModels.length === 0) return;

    comparisonAbortController.current?.abort();
    const controller = new AbortController();
    comparisonAbortController.current = controller;

    const packNames = boosterCards.map(c => c.name);
    const deckNames = pickedCards.map(c => c.name);
    setModelPredictions({});

    availableModels.forEach((model) => {
      fetchPredictions(currentSet, packNames, deckNames, model.model_id, controller.signal)
        .then((predictions) => {
          if (controller.signal.aborted) return;
          setModelPredictions(previous => ({ ...previous, [model.model_id]: predictions }));
        })
        .catch((e) => {
          if (!controller.signal.aborted && e?.name !== 'AbortError') {
            console.error(`Prediction failed for ${model.model_id}:`, e);
          }
        });
    });
  ...
  }, [showModelBoxes, boosterCards, pickedCards.length, availableModels.length, currentSet]);
      ...
      >
        ...
        {error ? (
          ...
          <DraftResults
            ...
            onRestartDraft={() => (lockedSetCode ? initializeDraft() : (window.location.href = '/sets'))}
          />
        ) : (
          <>
            {loading ? (
              ...
              <>
                {showModelBoxes && availableModels.length > 0 && (
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {availableModels.map((model) => {
                      const predictions = modelPredictions[model.model_id];
                      const top = predictions?.[0];
                      const holdout = model.metrics?.holdout?.top_1_accuracy;
                      const isShown = model.model_id === selectedModelId;
                      return (
                        <button
                          key={model.model_id}
                          type="button"
                        ...
                        >
                          <div className="flex items-baseline justify-between">
                            <span className="text-lg font-bold text-white">{versionLabel(model.version)}</span>
                            {typeof holdout === 'number' && (
                              <span className="text-xs text-gray-400">
                                {(holdout * 100).toFixed(1)}% on an unseen set
                              </span>
                            )}
                          </div>
                          <p className="mt-1 text-xs text-gray-500">
                            {VERSION_BLURBS[model.version] ?? model.description}
                          </p>
                          ...
                          <p className="mt-3 text-xs text-gray-500">
                            {isShown ? 'Its numbers are on the cards' : 'Click to show its numbers'}
                          </p>
                        </button>
                      );
                    })}
                ...
                <div>
                  <BoosterGrid
                    ...
                    aiPredictions={
                      showModelBoxes
                        ? (selectedModelId ? modelPredictions[selectedModelId] ?? null : null)
                        : (isAiPredictionEnabled && selectedCardId ? aiPredictions : null)
                    }
                  />
                </div>
